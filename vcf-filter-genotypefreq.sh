...
if('${INPUTMINGENFREQ}'<1){
	...
	GENOTYPES_COUNTS$N_Genotypes_ltMINGENFREQ<-rowSums(GENOTYPES_COUNTS[,2:11]>0 & GENOTYPES_COUNTS[,2:11]<'${INPUTMINGENFREQ}')
	#SNPs that have at least one genotype with frequency <MINGENFREQ.
	SNPltMINGENFREQ<-GENOTYPES_COUNTS$SNP[which(rowSums(GENOTYPES_COUNTS[,2:11]>0 & GENOTYPES_COUNTS[,2:11]<'${INPUTMINGENFREQ}')>0)]
	SNPgeMINGENFREQ<-GENOTYPES_COUNTS$SNP[!(GENOTYPES_COUNTS$SNP %in% SNPltMINGENFREQ)]
}
