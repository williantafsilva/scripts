...
##Description:
##Index reference genome FASTA file (.fasta or .fa) using STAR.

##Input $1: Output location.
##Input $2: Reference genome FASTA file (.fa or .fasta).
##Input $3: Reference genome GTF file (.gtf).
##Output: Directory with STAR index files.
...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J refFASTA-refGTF-STARindex-${<INPUTFILE>##*/} \
##    refFASTA-refGTF-STARindex.sh <OUTPUT LOCATION> <INPUT REFERENCE FASTA FILE> <INPUT REFERENCE GTF FILE>

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "refFASTA-refGTF-STARindex.sh") 
...
GTFFILE=$(readlink -f $3)

INPUTFILELOCATION=${INPUTFILE%/*}
INPUTFILENAME=${INPUTFILE##*/}
GTFFILELOCATION=${GTFFILE%/*}
GTFFILENAME=${GTFFILE##*/}

############################################################################
##OUTPUT:

OUTPUTDIRPREFIX=$(echo ${INPUTFILENAME} | sed 's/\.fasta.*$//' | sed 's/\.fa.*$//' | sed 's/-job[0-9].*$//')
OUTPUTDIRNAME=$(echo "${OUTPUTDIRPREFIX}.STARindex-job${JOBID}") 
