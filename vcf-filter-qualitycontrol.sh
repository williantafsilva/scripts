...
##Description:
##Perform basic quality filtering on a VCF file (.vcf.gz).
...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J vcf-filter-qualitycontrol-${<INPUTFILE>##*/} \
##    vcf-filter-qualitycontrol.sh <OUTPUT LOCATION> <INPUT VCF FILE>

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "vcf-filter-qualitycontrol.sh") 
...
OUTPUTFILE1NAME=$(echo "${INPUTFILEPREFIX}.qcfilter-job${JOBID}.vcf.gz") 
...
#INFO/DP=Combined read depth across samples.
