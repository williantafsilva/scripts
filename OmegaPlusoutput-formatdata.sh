...
##Description:
##Concatenate and format megaPlus output data to be used for plotting (.txt).

##Input $1: Output location.
##Input $2: Directory containing megaPlus output files (per chromosome).
...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J OmegaPlusoutput-formatdata-${<INPUTDIR>##*/} \
##    OmegaPlusoutput-formatdata.sh <OUTPUT LOCATION> <INPUT DIRECTORY> 

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "OmegaPlusoutput-formatdata.sh") 
...
OUTPUTFILENAME=$(echo "${OUTPUTFILEPREFIX}.OmegaPlusdata-job${JOBID}.txt") 
...
find ${INPUTDIR}/OmegaPlus_Report.* -maxdepth 0 | while read F ; do
	CHROM=$(echo "${F##*/}" | cut -d. -f2)
	cat ${F} | sed "s/^/${CHROM}\t/" >> ${TMPFILE}
done

##Sort output file and add header.
cat ${TMPFILE} | sort -V -k1,1 -k2,2n > ${OUTPUTFILE}
sed -i '1iChromosome\tPosition\tLikelihood' ${OUTPUTFILE}
