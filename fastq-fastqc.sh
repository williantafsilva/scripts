...
##Output: HTML file (.html) and ZIP file (.zip).
...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J fastq-fastqc-${<INPUTFILE>##*/} \
##    fastq-fastqc.sh <OUTPUT LOCATION> <INPUT FASTQ FILE> 
...
INPUTFILEPREFIX=$(echo ${INPUTFILENAME} | sed 's/\.fastq\.gz.*$//' | sed 's/\.fq\.gz.*$//')
