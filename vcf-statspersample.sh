...
##Description:
##Sort and index VCF file (.vcf or .vcf.gz).

##Input $1: Output location.
##Input $2: VCF file (.vcf or .vcf.gz).
##Output: VCF file stats per sample (.txt).
...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J vcf-statspersample-${<INPUTFILE>##*/} \
##    vcf-statspersample.sh <OUTPUT LOCATION> <INPUT VCF FILE> 

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "vcf-statspersample.sh") 
...
OUTPUTFILENAME=$(echo "${OUTPUTFILEPREFIX}.statspersample-job${JOBID}.txt") 
OUTPUTFILE=$(echo "${OUTPUTLOCATION}/${OUTPUTFILENAME}") 

############################################################################
##ACTIONS:

rtg vcfstats ${INPUTFILE} > ${OUTPUTFILE}
