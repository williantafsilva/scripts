...
##Description:
##Count reads per position throughout the genome.

##Input $1: Output location.
##Input $2: BAM file (.bam).
##Output: Text file (.txt) with the following columns: chromosome, position, #reads.
...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J bam-readcountperposition-${<INPUTFILE>##*/} \
##    bam-readcountperposition.sh <OUTPUT LOCATION> <INPUT BAM FILE> 

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "bam-readcountperposition.sh") 
...
OUTPUTFILENAME=$(echo "${OUTPUTFILEPREFIX}.readcountperposition-job${JOBID}.txt") 
OUTPUTFILE=$(echo "${OUTPUTLOCATION}/${OUTPUTFILENAME}") 

############################################################################
##ACTIONS:

samtools depth -a ${INPUTFILE} > ${OUTPUTFILE}
