...
##Description:
##Create index file of reference genome FASTA file using Samtools.

##Input $1: Output location.
##Input $2: FASTA reference genome (.fasta, .fa, .fna).
##Output: Samtools (faidx) index file (.fai).
...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J refFASTA-samtoolsindex-${<INPUTFILE>##*/} \
##    refFASTA-samtoolsindex.sh <OUTPUT LOCATION> <REFERENCE GENOME FASTA FILE>

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "refFASTA-samtoolsindex.sh") 
...
OUTPUTFILEPREFIX=$(echo ${INPUTFILENAME} | sed 's/\.fasta.*$//' | sed 's/\.fa.*$//' | sed 's/\.fna.*$//' | sed 's/-job[0-9].*$//')
OUTPUTFILENAME=$(echo "${OUTPUTFILEPREFIX}.samtoolsindex-job${JOBID}.fai") 
...
samtools faidx --fai-idx ${OUTPUTFILE} ${INPUTFILE}
