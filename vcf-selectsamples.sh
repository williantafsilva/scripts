...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J vcf-selectsamples-${<INPUTFILE>##*/} \
##    vcf-selectsamples.sh <OUTPUT LOCATION> <INPUT VCF FILE> <LIST OF SAMPLES> <SAMPLE GROUP CODE> 
...
OUTPUTFILE1NAME=$(echo "${OUTPUTFILEPREFIX}.select${SAMPLEGROUP}-job${JOBID}.vcf.gz") 
OUTPUTFILE2NAME=$(echo "${OUTPUTFILEPREFIX}.select${SAMPLEGROUP}-job${JOBID}.vcf.gz.tbi") 
...
bcftools view --samples ${SAMPLELIST} --min-ac=1 --write-index=tbi --output-type z --output ${OUTPUTFILE1} ${INPUTFILE}
