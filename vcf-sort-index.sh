...
##Output: Sorted VCF file (.vcf.gz) and index file (.tbi).
...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J vcf-sort-index-${<INPUTFILE>##*/} \
##    vcf-sort-index.sh <OUTPUT LOCATION> <INPUT VCF FILE> 

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "vcf-sort-index.sh") 
...
bcftools sort --temp-dir "${PATHTOPROJTMP}/" --write-index=tbi --output-type z --output ${OUTPUTFILE1} ${INPUTFILE}
