...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J vcf-compress-${<INPUTFILE>##*/} \
##    vcf-compress.sh <OUTPUT LOCATION> <INPUT VCF FILE> 

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "vcf-compress.sh") 
...
OUTPUTFILENAME=$(echo "${OUTPUTFILEPREFIX}.gz-job${JOBID}.vcf.gz") 
OUTPUTFILE=$(echo "${OUTPUTLOCATION}/${OUTPUTFILENAME}") 

############################################################################
##ACTIONS:

bcftools view -I ${INPUTFILE} -O z -o ${OUTPUTFILE}
##bgzip -c ${INPUTFILE} > ${OUTPUTFILE} ##Alternative using samtools.
