...
##Description:
##Sort and index BCF file and count variants.

##Input $1: Output location.
##Input $2: BCF file (.bcf).
##Output: Sorted BCF file and index files (.csi).
...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J bcf-sort-index-${<INPUTFILE>##*/} \
##    bcf-sort-index.sh <OUTPUT LOCATION> <INPUT BCF FILE> 

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "bcf-sort-index.sh") 
...
OUTPUTFILEPREFIX=$(echo ${INPUTFILENAME} | sed 's/\.bcf.*$//' | sed 's/-job[0-9].*$//')
OUTPUTFILE1NAME=$(echo "${OUTPUTFILEPREFIX}.sort-job${JOBID}.bcf") 
OUTPUTFILE2NAME=$(echo "${OUTPUTFILEPREFIX}.sort-job${JOBID}.bcf.csi") 
...
bcftools sort ${INPUTFILE} -Ob -o ${OUTPUTFILE1}
bcftools index -c ${OUTPUTFILE1} -o ${OUTPUTFILE2} ##Output .csi format.
