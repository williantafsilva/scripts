...
##Description:
##Concatenate and format SweepFinder2 output data to be used for plotting (.txt).

##Input $1: Output location.
##Input $2: Directory containing SweepFinder2 output files (per chromosome).
...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J SF2output-formatdata-${<INPUTDIR>##*/} \
##    SF2output-formatdata.sh <OUTPUT LOCATION> <INPUT DIRECTORY> 

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "SF2output-formatdata.sh") 
...
OUTPUTFILENAME=$(echo "${OUTPUTFILEPREFIX}.SF2data-job${JOBID}.txt") 
...
find ${INPUTDIR}/*.SF2output -maxdepth 0 | while read F ; do
	CHROM=$(echo "${F##*/}" | cut -d. -f1)
	tail -n+2 ${F} | sed "s/^/${CHROM}\t/" >> ${TMPFILE}
done

##Sort output file and add header.
cat ${TMPFILE} | sort -V -k1,1 -k2,2n > ${OUTPUTFILE}
sed -i '1iChromosome\tPosition\tLikelihood\tAlpha' ${OUTPUTFILE}
