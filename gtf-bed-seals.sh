...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J gtf-bed-seals-${<INPUTFILE>##*/} \
##    gtf-bed-seals.sh <OUTPUT LOCATION> <INPUT GTF FILE> 

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "gtf-bed-seals.sh") 
...
sed 's/[[:space:]]/\t/g' | sed 's/[;|"]//g' | sed 's/.*exon_number\t\([0-9]*\)\t.*/exon\1/g') | \
awk '{print $1"\t"$2"\t"$3"\t"$4"."$7"\t"$5"\t"$6}' | \
