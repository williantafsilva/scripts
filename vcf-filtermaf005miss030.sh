...
##    vcf-filtermaf005miss030.sh <OUTPUT LOCATION> <INPUT VCF FILE>

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "vcf-filtermaf005miss030.sh") 
...
OUTPUTFILE1NAME=$(echo "${INPUTFILEPREFIX}.filtermaf005miss030-job${JOBID}.vcf.gz") 
...
bcftools view --include 'MAF>0.05 & F_MISSING<0.30' --output-type z --write-index=tbi --output ${OUTPUTFILE1} ${INPUTFILE}
