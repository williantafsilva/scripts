...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J vcf-filtermafmiss-${<INPUTFILE>##*/} \
##    vcf-filtermaf005miss040.sh <OUTPUT LOCATION> <INPUT VCF FILE>

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "vcf-filtermaf005miss040.sh") 
...
OUTPUTFILE1NAME=$(echo "${INPUTFILEPREFIX}.filtermaf005miss040-job${JOBID}.vcf.gz") 
...
bcftools view --include 'MAF>0.05 & F_MISSING<0.40' --output-type z --write-index=tbi --output ${OUTPUTFILE1} ${INPUTFILE}
