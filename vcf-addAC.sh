...
##Description:
##Add AC field to VCF file (.vcf or .vcf.gz).
...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J vcf-addAC-${<INPUTFILE>##*/} \
##    vcf-addAC.sh <OUTPUT LOCATION> <INPUT VCF FILE> 

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "vcf-addAC.sh") 
...
OUTPUTFILENAME=$(echo "${OUTPUTFILEPREFIX}.addAC-job${JOBID}.vcf.gz") 
OUTPUTFILE=$(echo "${OUTPUTLOCATION}/${OUTPUTFILENAME}") 

############################################################################
##ACTIONS:

zcat ${INPUTFILE} | vcf-annotate --fill-AC-AN | bcftools view -O z -o ${OUTPUTFILE}
