...
##Input $2: Phased and indexed VCF file (.vcf or .vcf.gz).
##Input $3: Window size (bp).
##Output: Table file (.txt) containing likage disequilibrium measurements (D, D' and r^2).
...
##    --mail-type=ALL \
##    --mail-user=${MYEMAIL} \
##    -J vcf-linkagedisequilibrium-${<INPUTFILE>##*/} \
##    vcf-linkagedisequilibrium.sh <OUTPUT LOCATION> <INPUT VCF FILE> <WINDOW SIZE>

############################################################################
##SCRIPT NAME:

SCRIPTNAME=$(echo "vcf-linkagedisequilibrium.sh") 
...
OUTPUTFILEPREFIX=$(echo "${OUTPUTLOCATION}/${INPUTFILEPREFIX}.LD${WINDOWSIZE}bp-job${JOBID}") 
...
vcftools \
	--gzvcf ${INPUTFILE} \
	--hap-r2 \
	--ld-window-bp ${WINDOWSIZE} \
	--out ${OUTPUTFILEPREFIX} 

############################################################################
##SAVE CONTROL FILES:

if [[ -s "${OUTPUTFILEPREFIX}.hap.ld" ]]; then
...
Input window size: ${WINDOWSIZE}
Output file: ${OUTPUTFILEPREFIX}.hap.ld
