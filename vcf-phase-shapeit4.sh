...
SCRIPTNAME=$(echo "vcf-phase-shapeit4.sh") 
...
TMPDIR=$(echo "${OUTPUTLOCATION}/tmp-vcf-phase-shapeit4-job${JOBID}")
FILELIST=$(echo "${TMPDIR}/list-bcffiles.txt") 
mkdir -p ${TMPDIR}

##Phasing.
bcftools index -s ${INPUTFILE} | cut -f 1 | while read C ; do 
	if [[ $(grep "^${C}$" "${HAPLOIDLIST}" | wc -l) -gt 0 ]]; then
		echo "Haploid chromosome found: ${C}."
	else
		echo "Processing ${C}."
